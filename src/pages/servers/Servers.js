...
const Servers = () => {
  ...
  return (
    <div className={changedOtherSession && styles.disabled}>
      {changedOtherSession && <span style={{ fontSize: "20px" }}>Form updated in another session. Please reload!</span>}
      ...
      {!changedOtherSession && !loading && (
        <div>
          <button className={styles.btnSave} onClick={handleUpdateDoc} disabled={loading}>Save</button>
          <button className={styles.btnAdd} onClick={handleAddDoc} disabled={loading}>Add</button>
        </div>
      )}
    </div>
  );
